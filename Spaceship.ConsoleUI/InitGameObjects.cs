...
namespace SpaceImpact.ConsoleUI
{
    class InitGameObjects
    {
        ...
        public void InitAndStart()
        {
            ...
            SpaceshipEventMethods methods = new SpaceshipEventMethods();
            
            Control control = new Control();
            control.GetAction += methods.OnGetAction;
            ...
            enemy.EnemyHide += methods.OnEnemyHide;
            enemy.EnemyDraw += methods.OnEnemyDraw;
            
            Laser laser = new Laser();
            laser.LaserHide += methods.OnLaserHide;
            laser.LaserDraw += methods.OnLaserDraw;

            Space space = new Space(_minPointX, _minPointY,_maxPointX, _maxPointY, _minXBound, _minYBound, _minYBound + 2);
            space.BordersDraw += methods.OnBordersDraw;
            space.GameSpaceDraw += methods.OnGameSpaceDraw;

            Spaceship player = new Spaceship(_playerLife,_playerStartPointX, _playerStartPointY);
            player.DrawHealth += methods.OnDrawHealth;
            player.HideSpaceship += methods.OnHideSpaceship;
            player.DrawSpaceship += methods.OnDrawHero;

            Spaceship boss = new Spaceship(_bossLife, _bossStartPointX, _bossStartPointY);
            boss.DrawHealth += methods.OnDrawHealth;
            boss.HideSpaceship += methods.OnHideSpaceship;
            boss.DrawSpaceship += methods.OnDrawBoss;

            Game game = new Game(player, boss, laser, enemy, control, space, _scoreValueX, _scoreValueY,
                _playerHealthX, _playerHealthY, _bossHealthX, _bossHealthY);
            game.GameScoreUpdate += methods.OnGameScoreUpdate;
            game.ConfirmGameExit += methods.OnConfirmGameExit;
            game.GameIsOver += methods.OnGameExit;
            game.ClearInfoBlock += methods.OnClearInfoBlock;
            game.ShowCtrlButtons += methods.OnShowCtrlButtons;
            game.StartGame();
        }
